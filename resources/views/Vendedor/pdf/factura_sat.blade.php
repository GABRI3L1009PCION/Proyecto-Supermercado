<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="utf-8">
    <title>Factura SAT - {{ $pedido->id }}</title>
    <style>
        body{ font-family: DejaVu Sans, Arial, Helvetica, sans-serif; font-size:12px; color:#222; }
        .wrap{ width: 92%; margin: 0 auto; }
        .header{ display:flex; justify-content:space-between; align-items:center; border-bottom:2px solid #8C3A44; padding-bottom:8px; margin-bottom:10px;}
        .logo{ width:120px; height:60px; object-fit:contain }
        .title{ text-align:right }
        .title h1{ margin:0; font-size:20px; color:#722F37 }
        .muted{ color:#666 }
        .grid{ display:flex; gap:14px }
        .box{ flex:1; border:1px solid #ddd; border-radius:6px; padding:8px 10px; }
        h3{ margin:6px 0; color:#722F37; font-size:13px }
        table{ width:100%; border-collapse:collapse; margin-top:6px }
        th,td{ padding:6px 8px; border-bottom:1px solid #eee; }
        thead th{ background:#F9F2F3; color:#722F37; border-bottom:2px solid #D9A6A6; }
        .right{ text-align:right }
        .totales{ margin-top:8px; width:50%; margin-left:auto }
        .foot{ margin-top:10px; font-size:11px; color:#555 }
        .sat-line{ font-weight:700; color:#444 }
    </style>
</head>
<body>
<div class="wrap">

    <div class="header">
        <div>
            @if($logoBase64)
                <img class="logo" src="{{ $logoBase64 }}" alt="logo">
            @else
                <div style="font-weight:700; font-size:18px;">{{ $empresa['nombre'] }}</div>
            @endif
            <div class="muted">{{ $empresa['direccion'] }}</div>
            <div class="muted">Tel: {{ $empresa['telefono'] }} · {{ $empresa['email'] }}</div>
            <div class="muted">NIT: {{ $empresa['nit'] }}</div>
        </div>
        <div class="title">
            <h1>FACTURA</h1>
            <div><strong>Serie:</strong> {{ $serie }} &nbsp; <strong>No.:</strong> {{ $numero }}</div>
            <div><strong>Fecha:</strong> {{ optional($pedido->created_at)->format('d/m/Y H:i') }}</div>
        </div>
    </div>

    <div class="grid">
        <div class="box">
            <h3>Emisor</h3>
            <div><strong>{{ $empresa['nombre'] }}</strong></div>
            <div>{{ $empresa['direccion'] }}</div>
            <div>NIT: {{ $empresa['nit'] }}</div>
        </div>
        <div class="box">
            <h3>Receptor</h3>
            <div><strong>{{ optional($pedido->cliente)->name ?? 'Consumidor Final' }}</strong></div>
            <div>NIT/CUI: {{ $nitReceptor ?: 'CF' }}</div>
            <div>Tel: {{ optional($pedido->cliente)->telefono ?? '--' }}</div>
            <div>Correo: {{ optional($pedido->cliente)->email ?? '--' }}</div>
        </div>
    </div>

    <table>
        <thead>
        <tr>
            <th>Descripción</th>
            <th class="right">Cantidad</th>
            <th class="right">Precio</th>
            <th class="right">Subtotal</th>
        </tr>
        </thead>
        <tbody>
        @foreach($items as $it)
            @php
                $cant = (int) $it->cantidad;
                $pu   = (float) $it->precio_unitario;
            @endphp
            <tr>
                <td>{{ optional($it->producto)->nombre ?? 'Producto' }}</td>
                <td class="right">{{ $cant }}</td>
                <td class="right">Q {{ number_format($pu,2) }}</td>
                <td class="right">Q {{ number_format($cant*$pu,2) }}</td>
            </tr>
        @endforeach
        </tbody>
    </table>

    <table class="totales">
        <tr><td class="right">Subtotal:</td><td class="right">Q {{ number_format($subtotal,2) }}</td></tr>
        <tr><td class="right">IVA (12%):</td><td class="right">Q {{ number_format($iva,2) }}</td></tr>
        <tr><td class="right"><strong>Total:</strong></td><td class="right"><strong>Q {{ number_format($total,2) }}</strong></td></tr>
    </table>

    <div class="foot">
        <div class="sat-line">Documento tributario electrónico (simulado) – SAT Guatemala.</div>
        <div>Autorización/UUID: <em>SIM-{{ $serie }}-{{ $numero }}-{{ $pedido->id }}</em></div>
        <div>Este PDF es una representación impresa; para efectos demostrativos.</div>
    </div>
</div>
</body>
</html>

